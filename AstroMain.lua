...
local function updateRoom()
    local roomFolder = API.ws:FindFirstChild("CurrentRoom")
    if roomFolder then
        API.run.currentRoom = roomFolder:FindFirstChildWhichIsA("Model")
        if API.run.currentRoom then
            API.run.twisteds = API.run.currentRoom:FindFirstChild("Monsters")
            API.run.items = API.run.currentRoom:FindFirstChild("Items")
            API.run.generators = API.run.currentRoom:FindFirstChild("Generators")
        end
    end
end
			...
			if elevators then
				for _, elev in ipairs(elevators:GetChildren()) do
					if elev.Name == "Elevator" then 
						local door = elev:FindFirstChild("ElevatorDoor") or elev:FindFirstChild("DoorVisible") or elev:FindFirstChild("Door")
						if door then
