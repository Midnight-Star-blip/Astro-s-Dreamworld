...
pcall(function()
    for _, g in ipairs(game:GetService("CoreGui"):GetChildren()) do
        if g.Name:find("Astro") then g:Destroy() end
    end
end)
...
local API = { ws = workspace, plrs = Players, rst = ReplicatedStorage }

API.player = {}
API.player.plr = localPlayer
API.player.char = localPlayer.Character or localPlayer.CharacterAdded:Wait()
...
localPlayer.CharacterAdded:Connect(function(char)
    ...
    API.player.hum = char:WaitForChild("Humanoid")
    API.player.root = char:WaitForChild("HumanoidRootPart")
end)

API.run = {}
API.run.getCooldown = function(monster)
    if not monster then return 0 end
    for _, v in ipairs(monster:GetDescendants()) do
        if (v:IsA("NumberValue") or v:IsA("IntValue")) then
            local val = v.Value
            local name = v.Name:lower()
            if val >= 8 and val <= 18 and not name:find("patrol") and not name:find("speed") then
                return val
            end
        end
    end
...
AstroUI.Theme = {
	Background = Color3.fromRGB(75, 60, 175),
	Header = Color3.fromRGB(245, 245, 255),
	Panel = Color3.fromRGB(90, 70, 195),
	PanelLight = Color3.fromRGB(110, 85, 220),
	Accent = Color3.fromRGB(130, 105, 245),
	AccentSoft = Color3.fromRGB(80, 95, 210),
	Hover = Color3.fromRGB(150, 130, 255),
	Shadow = Color3.fromRGB(20, 15, 55),
	PanelStroke = Color3.fromRGB(160, 140, 255),
	SoftStroke = Color3.fromRGB(200, 190, 255),
	WhiteStroke = Color3.fromRGB(255, 255, 255),
	ButtonTop = Color3.fromRGB(140, 115, 250),
	Text = Color3.fromRGB(255, 255, 255),
	HeaderText = Color3.fromRGB(25, 20, 90),
	Muted = Color3.fromRGB(220, 215, 245),
...
function AstroUI.CreateWindow(options)
	...
	local guiSize = isMobile and UDim2.fromScale(0.93, 0.82) or UDim2.fromOffset(824, 482)
	...
	create("Frame", {Name = "HeaderBack", Position = UDim2.fromOffset(12, 0), Size = UDim2.new(1, -24, 0, 48), BackgroundColor3 = theme.Header, BorderSizePixel = 0, ZIndex = 1, Parent = self.Main}, {corner(12)})
	self.Header = create("Frame", {Name = "Header", Position = UDim2.fromOffset(12, 0), Size = UDim2.new(1, -24, 0, 48), BackgroundTransparency = 1, BorderSizePixel = 0, ZIndex = 4, Parent = self.Main})
	create("TextLabel", {Name = "Title", Position = UDim2.fromOffset(12, 0), Size = UDim2.new(0.7, 0, 1, 0), BackgroundTransparency = 1, Text = options.Title or "Astro's Dreamworld", TextColor3 = theme.HeaderText, Font = theme.DisplayFont, TextSize = isMobile and 22 or 30, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 5, Parent = self.Header})
	self.CloseButton = create("TextButton", {Name = "Close", AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -12, 0.5, 0), Size = UDim2.fromOffset(28, 28), BackgroundTransparency = 1, BorderSizePixel = 0, Text = "✕", TextColor3 = theme.HeaderText, Font = Enum.Font.GothamBold, TextSize = 18, ZIndex = 5, Parent = self.Header})

	
	self.Body = create("Frame", {Name = "Body", Position = UDim2.fromOffset(0, 52), Size = UDim2.new(1, 0, 1, -54), BackgroundColor3 = theme.Background, BorderSizePixel = 0, ZIndex = 2, Parent = self.Main}, {corner(14), stroke(theme.PanelStroke, 2, 0.2)})

	self.Sidebar = create("ScrollingFrame", {Name = "Sidebar", Position = UDim2.fromOffset(10, 10), Size = UDim2.fromOffset(isMobile and 138 or 175, 9999), BackgroundColor3 = theme.Panel, BorderSizePixel = 0, Active = true, CanvasSize = UDim2.fromOffset(0, 0), ScrollingDirection = Enum.ScrollingDirection.Y, ScrollBarThickness = 4, ScrollBarImageColor3 = theme.Accent, ZIndex = 3, Parent = self.Body}, {corner(12)})

	local sidebarLayout = create("UIListLayout", {Padding = UDim.new(0, 10), SortOrder = Enum.SortOrder.LayoutOrder, Parent = self.Sidebar})
	create("UIPadding", {PaddingTop = UDim.new(0, 12), PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(0, 10), Parent = self.Sidebar})
	sidebarLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() self.Sidebar.CanvasSize = UDim2.fromOffset(0, sidebarLayout.AbsoluteContentSize.Y + 30) end)

	self.Content = create("Frame", {Name = "Content", Position = UDim2.fromOffset(isMobile and 155 or 175, 10), Size = UDim2.new(1, -(isMobile and 165 or 187), 1, -20), BackgroundColor3 = theme.Panel, BorderSizePixel = 0, ZIndex = 3, Parent = self.Body}, {corner(12)})
	...
	if isMobile then
		...
		mobileGui.Name = "AstroMobileButtonGui"
		...
		local mobileBtn = create("TextButton", {
			...
			BackgroundColor3 = Color3.fromRGB(130, 90, 245),
			...
			ZIndex = 1000000,
			Parent = mobileGui
		}, {corner(40), stroke(Color3.fromRGB(255,255,255), 4, 0.1)})
				...
				if monFolder then
					for _, monster in ipairs(monFolder:GetChildren()) do
						pcall(function()
							...
							local nombre = string.gsub(monster.Name, "Monster", ""):gsub("Character", "")
							...
							if hasBigAbility and API and API.run and type(API.run.getCooldown) == "function" then
				...
				if ESPGen then
					...
					if gensFolder then
						for _, gen in ipairs(gensFolder:GetChildren()) do
							pcall(function()
								if gen.Name ~= "Generator" and not gen:FindFirstChild("BaseMachine") then return end
