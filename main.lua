local tctgeP = {"BXIWyxZYzA";"F3KRz";"6bJyWludxFh";"A9g";"7WKvUi51v57aw6"}
local 0oLQ94HY = function(...)
...
function Tab:CreateSlider(text, minValue, maxValue, defaultValue, callback)
	...
	local function setFromX(yX7lsUsh)
		local alpha = math.clamp((yX7lsUsh - bar.AbsolutePosition.X) / bar.AbsoluteSize.X, 0, 1)
...
function Tab:CreateKeybind(text, defaultKey, callback)
	...
	if isMobile then
		...
		btn.InputBegan:Connect(function(HXe8KGUf) 
			if HXe8KGUf.UserInputType == Enum.UserInputType.Touch then 
				mD = true 
				mS = HXe8KGUf.Position 
				mP = btn.Position 
			end 
		end)
		
		UserInputService.InputChanged:Connect(function(HXe8KGUf) 
			if mD and HXe8KGUf.UserInputType == Enum.UserInputType.Touch then 
				local d = HXe8KGUf.Position - mS 
				btn.Position = UDim2.new(mP.X.Scale, mP.X.Offset + d.X, mP.Y.Scale, mP.Y.Offset + d.Y) 
			end 
		end)
		
		UserInputService.InputEnded:Connect(function(HXe8KGUf) 
			if HXe8KGUf.UserInputType == Enum.UserInputType.Touch then 
				mD = false 
			end 
		end)
	...
	UserInputService.InputBegan:Connect(function(HXe8KGUf, p)
		if p then return end
		if listening and HXe8KGUf.UserInputType == Enum.UserInputType.Keyboard then
			currentKey = HXe8KGUf.KeyCode ~= Enum.KeyCode.Escape and HXe8KGUf.KeyCode or nil
			bindButton.Text = currentKey and currentKey.Name or loadstring(base64decode("Tm9uZQ=="))() 
			listening = false 
			bindButton.BackgroundColor3 = theme.Accent
		elseif currentKey and HXe8KGUf.KeyCode == currentKey then 
			callback() 
		end
	end)
...
local function GetCooldown(monster)
	...
	for _, v in ipairs(monster:GetDescendants()) do
		if (v:IsA(loadstring(base64decode("TnVtYmVyVmFsdWU="))()) or v:IsA(loadstring(base64decode("SW50VmFsdWU="))())) then
			...
			local Mx3mlV1x = v.Value
			
			
			if valName:find(loadstring(base64decode("cGF0cm9s"))()) or valName:find(loadstring(base64decode("c3BlZWQ="))()) or valName:find(loadstring(base64decode("d2Fsaw=="))()) then
				continue
			end
			
			if Mx3mlV1x >= 8 and Mx3mlV1x <= 18 then
				cooldownTimers[monster] = Mx3mlV1x
				return Mx3mlV1x
			end
		end
	end
	...
	local chaser = monster:FindFirstChild(loadstring(base64decode("Q2hhc2Vy"))()) or monster:FindFirstChild(loadstring(base64decode("QUk="))())
	if chaser then
...
local function clearESP()
 ...
 pcall(function()
		for _, obj in ipairs(workspace:GetDescendants()) do
			if obj.Name == loadstring(base64decode("QXN0cm9IaWdobGlnaHQ="))() or obj.Name == loadstring(base64decode("QXN0cm9UYWc="))() then
				obj:Destroy()
			end
		end
	end)
				...
				if itemsFolder then
					for _, item in ipairs(itemsFolder:GetChildren()) do
						pcall(function()
							...
							elseif nameLower:find(loadstring(base64decode("Y2hvY29sYXRlIGJveA=="))()) then
					...
					for _, gen in ipairs(gensFolder:GetChildren()) do
						pcall(function()
							...
							if not isCompleted then
								local lightRef = gen:FindFirstChild(loadstring(base64decode("TGlnaHRSZWZlcmVuY2U="))()) or baseMachine:FindFirstChild(loadstring(base64decode("TGlnaHRSZWZlcmVuY2U="))())
								if lightRef then
			...
			if elevators then
				for _, elev in ipairs(elevators:GetChildren()) do
					if elev.Name == loadstring(base64decode("RWxldmF0b3I="))() then 
						...
						local door = elev:FindFirstChild(loadstring(base64decode("RWxldmF0b3JEb29y"))())
						
						if door then
							makeESP(door, loadstring(base64decode("RG9vcg=="))(), Color3.fromRGB(230, 100, 220))
						end
					end
				end
			end
