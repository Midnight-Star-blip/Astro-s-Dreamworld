local GAn1rl = {"u7InoGcB";"fSUawI2DjP90";"UC4n";"4nMPEe4gs";"iP32vsL";"048YLoveu35";"faj98vfggE";"7iaLKmj1on";"7EtD4nBnwIphdvF81"}
local enqxJZzZ = function(...)
...
function Tab:CreateKeybind(text, defaultKey, callback)
	...
	if isMobile then
		...
		btn.InputBegan:Connect(function(DkoYJvRx) 
			if DkoYJvRx.UserInputType == Enum.UserInputType.Touch then 
				mD = true 
				mS = DkoYJvRx.Position 
				mP = btn.Position 
			end 
		end)
		
		UserInputService.InputChanged:Connect(function(DkoYJvRx) 
			if mD and DkoYJvRx.UserInputType == Enum.UserInputType.Touch then 
				local d = DkoYJvRx.Position - mS 
				btn.Position = UDim2.new(mP.X.Scale, mP.X.Offset + d.X, mP.Y.Scale, mP.Y.Offset + d.Y) 
			end 
		end)
		
		UserInputService.InputEnded:Connect(function(DkoYJvRx) 
			if DkoYJvRx.UserInputType == Enum.UserInputType.Touch then 
				mD = false 
			end 
		end)
	...
	UserInputService.InputBegan:Connect(function(DkoYJvRx, p)
		if p then return end
		if listening and DkoYJvRx.UserInputType == Enum.UserInputType.Keyboard then
			currentKey = DkoYJvRx.KeyCode ~= Enum.KeyCode.Escape and DkoYJvRx.KeyCode or nil
			bindButton.Text = currentKey and currentKey.Name or loadstring(base64decode("Tm9uZQ=="))() 
			listening = false 
			bindButton.BackgroundColor3 = theme.Accent
		elseif currentKey and DkoYJvRx.KeyCode == currentKey then 
			callback() 
		end
	end)
